...
//        view.backgroundColor = RGBCOLOR(237,237,237);
        ...
        cancelButton.frame = CGRectMake(20, 0, 40, 40);
        [cancelButton setTitle:@"取消" forState:UIControlStateNormal];
        [cancelButton setTitleColor:RGB(233, 88, 59) forState:UIControlStateNormal];
        [cancelButton addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
        [view addSubview:cancelButton];
        
        UIButton *chooseButton = [UIButton buttonWithType:UIButtonTypeCustom];
        chooseButton.frame = CGRectMake(ScreenWidth - 60, 0, 40, 40);
        [chooseButton setTitle:@"确定" forState:UIControlStateNormal];
        [chooseButton setTitleColor:RGB(51, 51, 51) forState:UIControlStateNormal];
        [chooseButton addTarget:self action:@selector(doneClick) forControlEvents:UIControlEventTouchUpInside];
...
//    self.frame = CGRectMake(0, ScreenHeight, ScreenWidth, 20);
}
...
//    lefttem.tintColor = [UIColor grayColor];
//    
...
- (void)remove {
    ...
    [self.delegate toobarDonBtnHaveClick];
...
- (void)doneClick{
    if (_pickerView) {
        ...
        [nextformatter setDateFormat:@"HH"];
        NSInteger when = [[nextformatter stringFromDate:date] integerValue];
        [nextformatter setDateFormat:@"mm"];
        NSInteger ss = [[nextformatter stringFromDate:date] integerValue];
        [nextformatter setDateFormat:@"ss"];
        NSInteger points = [[nextformatter stringFromDate:date] integerValue];
        
        
        _resultString=[NSString stringWithFormat:@"%ld-%ld-%ld %ld:%0.2ld:%0.2ld",(long)year,(long)month,(long)day,(long)when,(long)ss,(long)points];
